...
- [ ] проверить - Нашёл при ручной проверке (не моя регрессия): экран создания транзакции в гостевом режиме висит на спиннере вечно — TransactionViewModel ждёт GET /category и GET /account, которые гостевой режим блокирует. Это путь чтения, я менял только запись. Отдельная задача. Тестовые данные с эмулятора убрал.
- [ ] TransactionRepositoryImpl.update по-прежнему молча теряет смену счёта у транзакции, хотя UI её предлагает. Барьер от этого больше не зависит, но пользовательский баг остался — и чинить его надо вместе с пересчётом балансов обоих счетов.
