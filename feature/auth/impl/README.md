...
В `build.gradle.kts` этого модуля те же плейсхолдеры продублированы **пустыми заглушками**:
плагин карты навигации включает модулю `includeAndroidResources`, из-за чего AGP собирает
манифест unit-тестов и мержит в него манифест Yandex ID SDK — без значений мерж падает.
На AAR это не влияет: реальные значения по-прежнему подставляет `:app`
(см. [`docs/nav-graph.md`](../../../docs/nav-graph.md)).
