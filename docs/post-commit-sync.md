...
2. **Вложенные `runInTransaction` поддерживаются.** Если очередь уже есть в контексте, внутренний
   вызов присоединяется к открытой транзакции и **своей очереди не заводит**: отложенные действия
   попадают во внешнюю и уходят после её commit. Откат внутри откатывает операцию целиком —
   `TransactionRollbackException` намеренно не перехватывается на вложенном уровне, иначе
   вызывающий получил бы `Failure`, а данные всё равно откатились бы (результат разошёлся бы с
   состоянием). Это позволяет репозиторию обеспечивать атомарность своей записи независимо от
   того, открыл ли транзакцию вызывающий use case.
3. **Крэш процесса между commit и диспатчем** теряет немедленные пуши, но не данные:
   операция закоммичена в очереди исходящих (`docs/outbox.md`), и фоновый синк (WorkManager)
   отправит её при следующем прогоне. Немедленный пуш — оптимизация
