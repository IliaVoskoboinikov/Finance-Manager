...
| [`OutboxProcessor`](../core/data/src/main/java/soft/divan/financemanager/core/data/outbox/OutboxProcessor.kt) | какие записи брать, в каком порядке и когда остановиться |
| [`OutboxEntryHandler`](../core/data/src/main/java/soft/divan/financemanager/core/data/outbox/OutboxEntryHandler.kt) | судьба одной записи: подтвердить, повторить позже, увести в dead-letter |
...
| `dependencyKey` | **предшественника** — кто обязан уехать раньше | `localId` счёта этой транзакции |
...
### Порядок: граф зависимостей вместо глобальной очереди

Операции связаны не все со всеми, а **парами**. Ровно три вида связей:

| Ребро | Пример | Почему обязательно |
|---|---|---|
| прошлая операция **той же строки** | создание → правка → удаление одной транзакции | правка по несуществующему id вернёт `404` |
| операция **предшественника** | счёт → его транзакция | сервер отвергает транзакцию с неизвестным `accountId` |
| операция **зависящего от нас** | транзакции счёта → удаление счёта | транзакция приедет к удалённому счёту и получит `400` |

Третье ребро — обратное второму. Без него связь односторонняя: «транзакция после счёта»
соблюдалось, а «удаление счёта после его транзакций» — нет. Сценарий не выдуманный: локальное
удаление счёта **не каскадит** на его транзакции, поэтому в очереди свободно лежит
«создать счёт → создать его транзакцию → удалить счёт».

Больше ничего. Две транзакции одного счёта — **ровесники**: обе ждут счёт, но друг друга не ждут.

Предшественник записан в `dependencyKey`: счёт кладёт туда собственный `localId` (он ни от кого не
зависит), транзакция — `localId` своего счёта. Это ссылка на предшественника, **не** «номер
группы»: сериализовать всех членов группы было бы напрасно — на 50 транзакциях счёта выборка
отдавала бы по одной за проход.

Порядок держат два механизма:

1. **Барьер в выборке** (`getReadyToSend`): подзапрос `NOT EXISTS` отсекает запись, у которой есть
   незакрытый предшественник по любому из трёх рёбер. Одного `ORDER BY sequenceNo` не хватало —
   запись, ушедшая в backoff или удерживающая аренду, из выборки выпадала, и следующая операция
   той же строки её обгоняла. Цикл невозможен: блокировать может только запись с **меньшим**
   `sequenceNo`.
2. **Пропуск зависимых в прогоне**: если операция вернула временную ошибку, процессор запоминает
   её и пропускает всё, что с ней связано теми же рёбрами. Барьер этого знать не может — операция
   застряла уже после того, как выборка была сделана.

Проверка первого ребра намеренно **не ограничена группой**: если сущность сменит владельца, её
операции окажутся в разных группах, но порядок между ними обязан сохраниться.
...
- **Терминальная ошибка** очередь не блокирует: запись уходит в `FAILED` и в выборку не попадает.
  Вместе с ней в dead-letter уводятся **зависящие от неё** — по тем же двум рёбрам, не вся группа.
  Это один SQL-запрос, поэтому окна между отказом предшественника и отменой зависимых не
  существует. Каскад не трогает записи в `IN_PROGRESS`: их отправляет параллельный прогон, и он
  доложит об исходе сам — иначе его `scheduleRetry` отменил бы отмену.
- **Заблокированная сеть** (гость, нет сессии) останавливает прогон **целиком**: это состояние
  клиента, а не сервера, и остальные операции упрутся в ту же стену.
...
| 30 транзакций одного счёта | уезжали по одной за проход | уезжают одной пачкой следом за счётом |
| Удаление счёта при неуехавших его транзакциях | удаление обгоняло их → `400` → транзакции в dead-letter | удаление ждёт свои транзакции |
...
Выборка берётся один раз на проход, а барьер не выдаёт операцию, пока не закрыт её предшественник.
Значит успешная отправка **разблокирует** зависящие от неё операции — но в уже выбранный батч они
не попали.
...
| `core/data/.../outbox/OutboxProcessor.kt` | порядок, захват, проходы |
| `core/data/.../outbox/OutboxEntryHandler.kt` | исход одной записи: повторы, dead-letter, атомарное подтверждение |
