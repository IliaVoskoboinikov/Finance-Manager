...
Навигация построена на **Navigation 3** и следует тому же разделению `api` / `impl`:

- `core:feature-api` и `feature:*:api` содержат общие контракты и навигационные ключи
  (`NavKey`, аргументы экрана — поля ключа);
- `feature:*:impl` реализует сами экраны и регистрирует свои `NavEntry` в общем
  `entryProvider`;
- `app` связывает всё вместе: собирает граф из `Set<FeatureApi>` и владеет двумя back stack —
  корневым и отдельным на каждую вкладку нижней навигации.

Каждая фича определяет свой API‑контракт (ключи и их аргументы) в `api`‑модуле и реализацию
экранов в `impl`, что уменьшает связность между фичами и облегчает рефакторинг.
Подробности — [`navigation3.md`](./navigation3.md).
