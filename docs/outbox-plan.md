...
- [x] **SyncCoordinator short-circuit.** Шаги pull больше не отменяют друг друга: результаты
  собираются в переменные и складываются после (`&&` пропускал бы оставшиеся). Порядок
  category → account → transaction сохранён, `lastSyncTime` по-прежнему пишется только при полном
  успехе. Разбор очереди был развязан раньше (итерация 10). ✅ *(итерация 15)*
- [ ] **(Опц., масштаб/позже)** delta-pull по `lastSyncTime`-курсору, N+1 в pull транзакций,
  батч-push. Корректность не блокируют.
- [x] **Доки/качество.** Создан дизайн-док [outbox.md](./outbox.md) (+ ссылка в `docs/README.md`);
  обновлены `synchronization.md` (схема и шаги: pull-менеджеры + разбор очереди), README
  `core:data` и `core:database`. KDoc на всех новых классах. ✅ *(итерация 13)*
  <br>Осталось из необязательного: индикатор в UI (доменная поверхность готова — см. §2 выше).
