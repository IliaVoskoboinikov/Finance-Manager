...
- [ ] **Никогда не регенерить ключ на попытку** — на каждый ретрай тот же id (иначе дубль; это
  текущий баг).
- [ ] **Трактовать идемпотентные ответы как успех.** 2xx / 200-replay / 409-on-create →
  `SYNCED`; 404-on-delete → успех. Иначе запись зависает в `PENDING` (poison-loop).
