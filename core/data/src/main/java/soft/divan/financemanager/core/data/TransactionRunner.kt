...
interface TransactionRunner {
     ...
     * - вложенные вызовы допустимы: внутренний присоединяется к уже открытой транзакции,
     *   отложенные пуши уходят после commit **внешней**, а откат внутри откатывает всё
     *   целиком. Это позволяет репозиторию гарантировать атомарность своей записи независимо
     *   от того, обернул ли его вызывающий use case в транзакцию.
     */
